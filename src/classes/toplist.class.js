function isDisplayProcess(proc) {
    if (!proc) return false;
    const name = String(proc.name || "").toLowerCase();
    if (Number(proc.pid) === 0) return false;
    if (process.platform === "win32" && name.includes("system idle")) return false;
    return true;
}

function prepareProcessList(data) {
    let list = (data && data.list ? data.list : []).filter(isDisplayProcess);
