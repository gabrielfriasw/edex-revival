class FilesystemDisplay {
    constructor(opts) {
        ...
        this.render = async (originBlockList, isDiskView) => {
            ...
            blockList.forEach((e, blockIndex) => {
                ...
                let cmdPrefix = `if (window.keyboard.container.dataset.isCtrlOn == "true") {
                                edex.openPath(fsDisp.cwd[${blockIndex}].path);
                                appWindow.minimize();
                ...
                switch(e.type) {
                    ...
                    case "edex-theme":
                        ...
                        type = "eDEX Revival theme";
                        break;
                    case "edex-kblayout":
                        icon = this.edexIcons.kblayout;
                        type = "eDEX Revival keyboard layout";
                        break;
                    case "edex-settings":
                    case "edex-shortcuts":
                        icon = this.edexIcons.settings;
                        type = "eDEX Revival config file";
                        break;
                    case "system":
                        icon = this.edexIcons.settings;
                        break;
                    case "edex-themesDir":
                        ...
                        type = "eDEX Revival themes folder";
                        break;
                    case "edex-kblayoutsDir":
                        icon = this.edexIcons.kblayoutsDir;
                        type = "eDEX Revival keyboards folder";
                        break;
                    default:
        ...
        if (window.performance.navigation.type === 0) {
            let startupCwd = window.term[window.currentTerm].cwd || window.settings.cwd;
            if (typeof startupCwd === "string" && startupCwd.startsWith("FALLBACK |-- ")) {
                startupCwd = startupCwd.slice(13);
                this._noTracking = true;
            }
            this.readFS(startupCwd);
        }
