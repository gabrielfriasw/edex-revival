class LocationGlobe {
    ...
    addRandomConnectedMarkers() {
        if (!window.isGlobeConnectionMode || !window.isGlobeConnectionMode()) return false;
    ...
    updateLoc() {
        if (!window.isGlobeLiveMode || !window.isGlobeLiveMode() || window.mods.netstat.offline) {
            document.querySelector("div#mod_globe").setAttribute("class", "offline");
            document.querySelector("i.mod_globe_headerInfo").innerText = this.mode() === "reduced" ? "(REDUCED)" : "(OFFLINE)";
    ...
    async updateConOnlineConnection() {
        if (!window.isGlobeLiveMode || !window.isGlobeLiveMode()) return false;
    ...
    updateConns() {
        if (!window.mods.globe.globe || window.mods.netstat.offline || !window.isGlobeConnectionMode || !window.isGlobeConnectionMode()) return false;
